...
for img in tqdm_notebook(col_images):  # 각각의 도로 사진에 대해서...
    ...
    try:
        ...
        right_xs = right_lane(y_values)
        """
        양쪽 차선이 모두 탐지 안되면 오류가 발생하는 듯...
        이 경우, 직전에 사용한 양쪽 차선을 모두 그대로 재사용 하자
        """
        print('Type Error가 발생했습니다. Error at INDEX : %d' % cnt)
        #plt.imshow(dmy) #plt.show() #assert False, "Type Error 가 발생했습니다."
