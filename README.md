...
* TODO
  * (완료) TypeError 발생하면 차선 탐지 못한 것과 동일하게 취급하고, 양쪽 차선 모두 직전에 탐지한 차선을 재활용 하도록 구현하자
  * 실행 오류 : 
    * v4, v5 파일이 원래는 jupyter notebook 이었는데, python 형식으로 다운받았다.
    * Visual Code에서 코드를 실행하면 오류가 발생하고,
    * PowerShell에서 실행하면 오류가 발생하지 않는다.
    * 왜지?
  * set3
    * set3 에서는 왼쪽 차선을 탐지 못하고, 블럭(?) 같은걸 탐지한다
    * 그리고 차선이 아닌 선을 너무 많이 탐지한다
    * slope_max=2로 설정하니까, 그래도 성능이 조금 나아진 것 같다.	
